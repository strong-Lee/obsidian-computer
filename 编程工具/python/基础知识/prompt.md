...
6. **面试话术**：如果有相关的性能坑或面试常考点，请专门用一个标签（如 #面试Tips）标出，并告诉我如何向面试官解释权衡（Trade-offs）。

**示例参考**：
```python
# 二分查找维护 - 算法优化 
# 场景：你维护一个“排行榜”，必须始终保持有序。 
import bisect 
scores = [60, 70, 80, 90] 
# ❌ 错误做法 (PHP/普通习惯)： 
# scores.append(75) -> O(1) 
# scores.sort() -> O(N log N) 
# 后果：数据量大时，每次插入都重排，性能雪崩。 
# ✅ 正确做法 (Pythonic)： 
# 动作：O(N) 插入。 
bisect.insort(scores, 75) 
print(scores) # [60, 70, 75, 80, 90] 
# 💡 面试官视角 (Interview Corner)： 
# 问题：“如何在一个大列表中插入元素并保持有序？” 
# 回答策略： # 1. "首选 bisect 模块。" (展示你知道标准库) 
# - "它是 C 实现的二分查找，可以 O(log N) 找到位置。" 
# 2. "但是，我要强调底层的代价。" (展示你懂 list 源码) 
# - "虽然找位置快，但 list.insert 本身是 O(N) 的。" 
# - "因为 list 是连续内存，插入中间会导致后面的元素全部都要 memmove 后移。" 
# 3. "架构权衡：" (展示架构思维) 
# - "如果是写少读多，用 bisect 没问题。" 
# - "如果是高频插入的海量数据，List 就不行了。我会考虑用跳表 (SkipList) 或 平衡树 (如 B-Tree) 结构。"
```
