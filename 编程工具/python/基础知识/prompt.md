...
**特别注意**：用户是一位有经验的 PHP 后端工程师（无 C++/Java 背景，计算机原理知识需复习）。你的教学目标是帮他建立完整的底层认知，而不仅仅是罗列知识点。
...
1. 内存模型与对象机制 (Memory & Objects) 
	- **C 结构体解剖**：请画出 `PyListObject` 的内存布局图（用文字图表），详细解释 `ob_item` 是什么？ 
	- **指针数组详解**：用户熟悉 PHP 的数组（HashTable），请对比解释 Python List 为什么是“存指针的数组”？
	- **多态实现的代价**：为什么 List 能存不同类型？具体到内存里，存 `1` 和 `"hello"` 时，List 的那个格子（Slot）里到底放的是什么？ 
	- **浅拷贝本质**：用内存地址的变化（如 `id()`）举例，说明 `new = old[:]` 时，到底复制了什么，没复制什么？ 
2. 动态扩容算法 (The Resizing Algorithm) 
	- **O(1) 的秘密**：请用数学公式解释，为什么拿到索引 `i` 就能立马算出来内存地址？ 
	- **扩容全过程**：当 `append` 触发扩容时，请分步骤描述：`malloc` 申请新内存 -> `memcpy` 搬运指针 -> 释放旧内存 的过程。 
	- **超额分配 (Over-allocation)**：请列出具体的扩容系数（Growth Factor）。为什么不是“要多少给多少”？这样做是为了解决什么性能问题？

3. 硬件性能与缓存友好性 (Hardware & Performance) 
	- **CPU 缓存视角**：请先通俗解释 **“缓存行 (Cache Line)”** 和 **“局部性原理”**。 - **Indirection (间接寻址) 的代价**：对比 C 数组（连续数据）和 Python List（连续指针+分散数据）。为什么 CPU 读取 List 会频繁发生 **Cache Miss**？这对性能有多大影响？ 
	- **SIMD 缺失**：解释为什么分散在堆内存的数据无法利用 CPU 的并行计算指令？ 

4. 垃圾回收机制 (GC Internals) 
	- **引用计数的增减**：当 `lst.append(obj)` 和 `lst.pop()` 时，`obj` 身上的引用计数器发生了什么具体的数字变化？ 
	- **sys.getsizeof 的误区**：为什么 `sys.getsizeof([1, 2, 3])` 返回的大小比我们预期的要小？它到底算了谁的大小？漏算了谁？
