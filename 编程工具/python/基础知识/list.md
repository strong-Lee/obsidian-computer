...
2. 快速构建： nums = [0] * 100
3. **切片复制：** new_lst = old_lst[:] 快速浅拷贝。
4. **步长切片：** lst[::2] 每隔一个取一个。
5. **列表反转：** lst[::-1] 快速生成反转列表。
6. **检查存在：** if x in lst: 判断元素是否存在。
7. **追加元素：** lst.append(x) 在末尾添加。
8. **扩展列表：** lst.extend(other_lst) 合并两个列表。
9. **统计次数：** lst.count(x) 统计 x 出现的次数。
10. **获取索引：** lst.index(x) 获取 x 第一次出现的下标。
11. **排序：** lst.sort() (原地排序) 或 sorted(lst) (返回新列表)。
## 2. 进阶高级
